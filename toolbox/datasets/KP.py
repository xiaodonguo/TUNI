...
class KP(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgb_path = f"{parts[0]}/{parts[1]}/visible/{parts[2]}"
        thermal_path = f"{parts[0]}/{parts[1]}/lwir/{parts[2]}"
        ...
        image = Image.open(os.path.join(self.root, 'images', rgb_path).replace('png', 'jpg'))
        thermal = Image.open(os.path.join(self.root, 'images', thermal_path).replace('png', 'jpg')).convert('RGB')
        label = Image.open(os.path.join(self.root, 'labels', image_path))
...
if __name__ == '__main__':
    ...
    class_weight = classweight.get_weight(train_loader, 19)
