...
class IRSeg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgb_path = os.path.join(self.root, 'seperated_images', image_path + '_rgb.png')
        t_path = os.path.join(self.root, 'seperated_images', image_path + '_th.png')
        label_path = os.path.join(self.root, 'labels', image_path + '.png')

        image = np.array(cv2.imread(rgb_path, cv2.COLOR_BGR2RGB), dtype=None)
        thermal = np.array(cv2.imread(t_path, cv2.IMREAD_GRAYSCALE), dtype=None)
        label = np.array(cv2.imread(label_path, cv2.IMREAD_GRAYSCALE), dtype=np.uint8)
        thermal = cv2.merge([thermal, thermal, thermal])
        ...
        image = self.normalize(image).transpose(2, 0, 1)
        thermal = self.normalize(thermal).transpose(2, 0, 1)

        sample['image'] = torch.from_numpy(np.ascontiguousarray(image)).float()
        sample['thermal'] = torch.from_numpy(np.ascontiguousarray(thermal)).float()
        sample['label'] = torch.from_numpy(np.ascontiguousarray(sample['label'])).long()
...
if __name__ == '__main__':
    ...
    path = '/home/guoxiaodong/code/seg/configs/irseg.json'
