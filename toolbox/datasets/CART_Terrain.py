...
class Terrain(data.Dataset):

    def __init__(self, cfg, mode='train', do_aug=True):

        assert mode in ['train', 'val', 'test', 'trainval'], f'{mode} not support.'
        self.mode = mode
        ...
        with open(os.path.join(self.root, f"rgbt_splits/Terrain_split/{cfg['terrain']}", f'rgb_{mode}.txt'), 'r') as f:
            for line in f:
                paths = line.strip().split(',')
                self.rgb_infos.append(paths[0].strip())
                self.label_infos.append(paths[1].strip())

        with open(os.path.join(self.root, f"rgbt_splits/Terrain_split/{cfg['terrain']}", f'thermal16_{mode}.txt'), 'r') as f:
            for line in f:
                paths = line.strip().split(',')
...
if __name__ == '__main__':
    ...
    path = "/home/ubuntu/code/wild/configs/CART_Terrain1.json"
    with open(path, 'r') as fp:
        cfg = json.load(fp)

    dataset = Terrain(cfg, mode='test', do_aug=True)
    print(len(dataset))
    # # train/val/test
    # # 1731/272/279
