...
class Decoder_MLP_plus(nn.Module):
    def __init__(self,
        ...
        super(Decoder_MLP_plus, self).__init__()
        ...
        self.linear_low = nn.Sequential(
            nn.Conv2d(in_channels=embedding_dim * 2, out_channels=embedding_dim, kernel_size=1),
        ...
        self.linear_pred_fuse = nn.Conv2d(embedding_dim, self.num_classes, kernel_size=1)

        self.f_pre = nn.ModuleList()
        for i in range(4):
            self.f_pre.append(nn.Conv2d(embedding_dim, self.num_classes, kernel_size=1))
    ...
    def forward(self, inputs):
        ...
        f = [_c1, _c2, _c3, _c4]
        fuse = self.linear_fuse(torch.cat((_c1, _c2, _c3, _c4), dim=1))
        fuse = self.dropout(fuse)
        fuse_out = self.linear_pred_fuse(fuse)
        if self.training == True:
            f_out = []
            for i in range(4):
                f_out.append(self.f_pre[i](f[i]))
            return fuse_out, f_out
        else:
            return fuse_out
...
if __name__ == '__main__':
    ...
    decoder = Decoder_MLP_plus(in_channels=[32, 64, 128, 256], embed_dim=256, num_classes=12).eval()
    out = decoder(input)
    print(out.shape)
    # for i in out:
    #     for p in i:
